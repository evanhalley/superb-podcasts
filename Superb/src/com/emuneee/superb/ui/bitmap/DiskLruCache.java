...
package com.emuneee.superb.ui.bitmap;
...
public class DiskLruCache {
	...
	/**
	 * Deletes the file with the specified key from the cache
	 * 
	 * @param key
	 *            key of the file to delete
	 */
	public void remove(String key) {
		String entry = mLinkedHashMap.get(key);
		if (entry != null) {
	...
	public String getPath(String key) {
		synchronized (mLinkedHashMap) {
			...
			if (file != null) {
				...
				if (new File(existingFile).exists()) {
					put(key, existingFile);
					return new File(existingFile).getAbsolutePath();
				}
			}
			return null;
		}
	...
	public static String createFilePath(File cacheDir, String key) {
		StringBuilder builder = new StringBuilder(cacheDir.getAbsolutePath());
		builder.append(File.separator).append(CACHE_FILENAME_PREFIX);
		builder.append(key.hashCode()).append(".jpg");
